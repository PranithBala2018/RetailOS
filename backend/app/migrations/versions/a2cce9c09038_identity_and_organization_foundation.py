...
Revision ID: a2cce9c09038
Revises:
Create Date: 2026-07-21 18:46:08.689570
...
revision: str = "a2cce9c09038"
down_revision: str | Sequence[str] | None = None
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

# NOTE: this migration is hand-adjusted from the autogenerate output.
# `use_alter=True` on a ForeignKeyConstraint tells SQLAlchemy's DDL
# compiler to omit that constraint from an inline CREATE TABLE — it does
# NOT, by itself, also emit the deferred ALTER TABLE (that only happens
# automatically via `MetaData.create_all()`'s dependency-sorted DDL
# sequence, not via Alembic's per-table `op.create_table()`). The four
# circular back-references below (branches<->users<->warehouses,
# refresh_tokens self-reference) are therefore added explicitly via
# `op.create_foreign_key()` after every table exists, and dropped before
# their tables in `downgrade()`. Verified end-to-end against a live
# Postgres instance — see docs/adr/0003 for the tenant-isolation context
# this schema was designed under.
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("default_tax_percent", sa.Numeric(precision=5, scale=2), nullable=True),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["company_id"], ["companies.id"], name=op.f("fk_branches_company_id_companies")
        ),
    ...
    op.create_table(
        ...
        sa.Column("id", sa.UUID(), nullable=False),
        sa.ForeignKeyConstraint(
            ["company_id"], ["companies.id"], name=op.f("fk_audit_logs_company_id_companies")
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["branch_id"], ["branches.id"], name=op.f("fk_user_branches_branch_id_branches")
        ),
        sa.ForeignKeyConstraint(
            ["user_id"], ["users.id"], name=op.f("fk_user_branches_user_id_users")
        ),
    ...
    op.create_table(
        ...
        sa.Column("updated_by", sa.UUID(), nullable=True),
        sa.ForeignKeyConstraint(
            ["branch_id"], ["branches.id"], name=op.f("fk_warehouses_branch_id_branches")
        ),
        sa.ForeignKeyConstraint(
            ["company_id"], ["companies.id"], name=op.f("fk_warehouses_company_id_companies")
        ),
    ...
    # --- hand-added: circular back-references, deferred until every
    # target table exists (see the module docstring above) ---
    op.create_foreign_key(
        "fk_branches_default_warehouse_id",
        "branches",
        "warehouses",
...
def downgrade() -> None:
    """Downgrade schema."""
    # --- hand-added: drop the circular back-references before the
    # tables they reference are dropped ---
    op.drop_constraint("fk_refresh_tokens_replaced_by", "refresh_tokens", type_="foreignkey")
    op.drop_constraint("fk_users_default_branch_id", "users", type_="foreignkey")
    op.drop_constraint("fk_branches_manager_user_id", "branches", type_="foreignkey")
    op.drop_constraint("fk_branches_default_warehouse_id", "branches", type_="foreignkey")
